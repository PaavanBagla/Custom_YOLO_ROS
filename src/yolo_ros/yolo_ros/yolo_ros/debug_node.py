...
class DebugNode(LifecycleNode):
    ...
    def draw_box(
        ...
        size_y = max(box_msg.size.y, min_size)
        ...
        if track_id:
            try:
                # Convert string ID to int to allow math operations
                numeric_id = int(track_id)
                
                # Logic: Alternates every 2 IDs to separate clusters of Evens and Odds
                if (numeric_id // 2) % 2 != 0:
                    text_y = min(max_pt[1] + 18, cv_image.shape[0] - 10)
            except ValueError:
                # If ID is a UUID string that can't be an int, just stay above
                pass

        text_pos = (min_pt[0], text_y)
        label_color = color # default
        if "Red" in label: label_color = (0, 0, 255)
